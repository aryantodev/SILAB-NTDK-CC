...
function EditProfileKepala() {
  ...
  // 1. LOAD DATA DARI DATABASE (DENGAN SINKRONISASI JALUR AWS S3)
  useEffect(() => {
    if (!token) {
      history.push("/login");
      return;
  ...
  const handleChange = (e) => {
    ...
    if (name === "institusi") return; // Institusi bersifat read-only bagi kepala
  ...
  const handleAvatarChange = (e) => {
    ...
    if (file) {
      setAvatarFile(file);
      setPreviewAvatar(URL.createObjectURL(file)); // Preview instan lokal di browser
    }
  };

  // 2. FUNGSI SIMPAN DENGAN TRANSMISI DATA KE BACKEND -> AWS S3
  const handleSave = async () => {
    ...
    if (avatarFile) {
      dataToSend.append("avatar", avatarFile); // Menggunakan key 'avatar' sesuai validasi backend mutakhir
    }
    ...
    try {
      ...
      // Jika backend merespons dengan URL S3 baru, sinkronkan ke preview screen
          ...
          <div className="col-md-8 col-lg-6">
            ...
            <div className="card shadow-sm border-0 overflow-hidden" style={{ borderRadius: "24px", backgroundColor: "#FFF" }}>
              ...
              <div className="p-4 text-center text-white" style={{ backgroundColor: "#4A3A2E" }}>
                <h4 className="fw-bold mb-1">Pengaturan Akun Kepala</h4>
                <p className="mb-0 small text-white-50">Lengkapi data profil struktural dan otoritas utama laboratorium</p>
              </div>
              ...
              <div className="card-body p-4">
                ...
                <div className="text-center mb-4">
                  ...
                  <div className="mt-3">
                    <h5 className="fw-bold mb-0" style={{ color: "#4A3A2E" }}>{formData.full_name || formData.name || "Kepala Baru"}</h5>
                ...
                <div className="mb-3">
                  ...
                  <input type="text" className="form-control rounded-3 py-2 border-secondary-subtle" name="full_name" value={formData.full_name} onChange={handleChange} placeholder="Contoh: Dr. Aryanto Pratama, M.Si" required />
                </div>
                ...
                <div className="mb-3">
                  ...
                  <small className="text-muted" style={{ fontSize: "11px" }}>Institusi terkunci otomatis khusus untuk pimpinan structural kepala laboratorium.</small>
                </div>
                ...
                <div className="mb-3">
                  ...
                  <textarea className="form-control rounded-3 border-secondary-subtle" name="bio" value={formData.bio} onChange={handleChange} rows={3} placeholder="Tulis rincian atau deskripsi keahlian kepemimpinan laboratorium Anda (opsional)..." />
                </div>
